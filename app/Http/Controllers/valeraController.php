...
class valeraController extends Controller
{
  ...
  public function guardar_valeras(Request $request)
  {
    ...
    public function asignarvaleraxdistribuidor(Request $request){
      ...
      $asignacionvalera->idvalera = $request->get('valera');
...
public function obtenerdesglosedeplazos(Request $request)
{
  ...
  $pagosnom = DB::select('CALL obtenerquincena()');
  $varprimeraquincena = collect($pagosnom);

      foreach($varprimeraquincena as $primeraquincnea)
      {
        $varprimeraquincena =  $primeraquincnea->fechaquincena;
        break;
      }
...
if($tprestamovalesenc->save())
      ...
      $idultpre= $prestamd->id;
                ...
                for($i=0; $i<$plazos; $i++)
                {
                  $resta_saldo=$resta_saldo-$pago_quincenal_total;
                        ...
                        if($diaprimeraquincena == 15)
                        {
                          ...
                          $det = new prestamos_valesdet();
                          $det->idprestamo_vales=$idultpre;
                          $det->plazos=$varplazo;
                          $det->fecha_pago=$varprimeraquincena;
                          $det->pago_quincenal = $pago_quincenalredondeado;
                          $det->coberturax_plazo=$coberturaxplazo;
                          $det->pago_total=$pago_quincenal_total;
                          $det->otros1=0;
                          $det->otros2=0;
                          $det->saldo_nuevo=$resta_saldo;
                          $det->created_at=$fecha;
                          $det->save();
                ...
                if($tipo_odp==6)
                {
                  ...
                  if($prestamoenc->save())
                  {
                    ...
                    if($restacapital->save())
                    {
                      ...
                      $pdf = \PDF::loadView('Vales.Gestion.ClienteFinal.vistadoc',compact('datoclienteenc','datosprestamodet','pago_total','total_plazantes_redodeo','totalcentavos','pago_quincenal_total','pago_quincenalredondeado','fecha','tipoodp','total_entregacliente','monto','var'));
                ...
                else
                {
                  ...
                  if($prestamoenc->save()){
                    ...
                    {
                      $var = $this->obtenerdatosusuariologueado($idusuario);
                      $pdf = \PDF::loadView('Vales.Gestion.ClienteFinal.vistadoc',compact('datoclienteenc','datosprestamodet','pago_total','total_plazantes_redodeo','totalcentavos','pago_quincenal_total','pago_quincenalredondeado','odpunido','fecha','tipoodp','total_entregacliente','monto','var'));
                      return $pdf->download("Tabla de amortizacion #prestamo $idultpre.pdf");
                    }
                    else
                    {
  ...
  else{echo "prestamosenc"; }
    } 
    else{echo "clientevales"; } 
  }
}
